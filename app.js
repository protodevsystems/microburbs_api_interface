...
function switchPlaygroundFeature(featureName) {
    ...
    if (featureName === 'tinder' && currentProperties.length > 0 && swipeStack.length === 0) {
        ...
        generateBattle();
    } else if (featureName === 'scores') {
        generateScoreDashboard();
    }
}
...
// Generate Score Dashboard (called when switching to scores sub-tab)
function generateScoreDashboard() {
    console.log('Generating Score Dashboard');
    
    if (!currentProperties || currentProperties.length === 0) {
        document.getElementById('scoredPropertiesGrid').innerHTML = '<p style="color: var(--text-secondary); text-align: center; padding: 3rem;">No properties to score. Please search for properties first.</p>';
        return;
    }
    
    // Calculate scores for all properties
    scoredProperties = currentProperties.map((property, index) => {
        return calculatePropertyScore(property, index);
    });
    ...
    currentFilteredScores = [...scoredProperties];
    
    // Update UI
    updateScoreSummary();
    renderDeansList();
    renderScoredProperties();
}

// Calculate Property Score
function calculatePropertyScore(property, index) {
    const scores = {
        propertyIndex: index,
        property: property,
        categories: {}
    ...
    // 1. Value Score (0-100) - Based on price relative to average
    const avgPrice = currentProperties.reduce((sum, p) => sum + (p.price || 0), 0) / currentProperties.length;
    const priceRatio = property.price / avgPrice;
    scores.categories.value = Math.max(0, Math.min(100, 100 - ((priceRatio - 1) * 50)));
    
    // 2. Location Score (0-100) - Based on land size
    const landSize = parseFloat(property.attributes?.land_size) || 0;
    const avgLandSize = currentProperties.reduce((sum, p) => sum + (parseFloat(p.attributes?.land_size) || 0), 0) / currentProperties.length;
    const landRatio = landSize / (avgLandSize || 1);
    scores.categories.location = Math.max(0, Math.min(100, landRatio * 50 + 25));
    
    // 3. Property Score (0-100) - Based on beds, baths, garage
    const beds = property.attributes?.bedrooms || 0;
    const baths = property.attributes?.bathrooms || 0;
    const garage = property.attributes?.garage_spaces || 0;
    const propertyPoints = (beds * 15) + (baths * 15) + (garage * 10);
    scores.categories.property = Math.min(100, propertyPoints);
    
    // 4. Market Score (0-100) - Based on days on market (fresher is better)
    const listingDate = property.listing_date ? new Date(property.listing_date) : null;
    const daysOnMarket = listingDate ? Math.floor((new Date() - listingDate) / (1000 * 60 * 60 * 24)) : 365;
    scores.categories.market = Math.max(0, Math.min(100, 100 - (daysOnMarket / 3.65))); // 100 for new, 0 for 365+ days
    
    // 5. Investment Score (0-100) - Price per bedroom efficiency
    const pricePerBed = beds > 0 ? property.price / beds : property.price;
    const avgPricePerBed = currentProperties.reduce((sum, p) => {
        const b = p.attributes?.bedrooms || 1;
        return sum + (p.price / b);
    }, 0) / currentProperties.length;
    const ppbRatio = pricePerBed / avgPricePerBed;
    scores.categories.investment = Math.max(0, Math.min(100, 100 - ((ppbRatio - 1) * 50)));
    
    // 6. Overall Appeal (0-100) - Holistic rating based on completeness and quality
    const hasDescription = property.attributes?.description ? 20 : 0;
    const hasLandSize = landSize > 0 ? 20 : 0;
    const hasBuildingSize = property.attributes?.building_size ? 20 : 0;
    const propertyQuality = (beds >= 3 ? 20 : beds * 6) + (baths >= 2 ? 20 : baths * 10);
    scores.categories.appeal = hasDescription + hasLandSize + hasBuildingSize + propertyQuality;
    
    // Calculate overall score (average of all categories)
    const categoryScores = Object.values(scores.categories);
    scores.overallScore = categoryScores.reduce((sum, score) => sum + score, 0) / categoryScores.length;
    
    // Calculate GPA (4.0 scale)
    scores.gpa = (scores.overallScore / 100) * 4.0;
    
    // Assign letter grade
    scores.grade = getLetterGrade(scores.overallScore);
...
function updateScoreSummary() {
    const avgScore = scoredProperties.reduce((sum, p) => sum + p.overallScore, 0) / scoredProperties.length;
    const topScore = Math.max(...scoredProperties.map(p => p.overallScore));
    ...
    const distribution = Object.entries(gradeCounts)
        ...
        .join(', ');
    document.getElementById('gradeDistribution').textContent = distribution;
}

// Render Dean's List
function renderDeansList() {
    const deansList = scoredProperties.filter((p, index) => {
        // Top 10% of properties
        return index < Math.ceil(scoredProperties.length * 0.1);
    });
    
    if (deansList.length === 0) {
        document.getElementById('deansListSection').style.display = 'none';
        return;
    }
    
    document.getElementById('deansListSection').style.display = 'block';
    const grid = document.getElementById('deansListGrid');
    grid.innerHTML = '';
    
    deansList.forEach(scored => {
        const card = createScoredPropertyCard(scored, true);
        grid.appendChild(card);
    });
}

// Render Scored Properties
function renderScoredProperties() {
    const grid = document.getElementById('scoredPropertiesGrid');
    grid.innerHTML = '';
    
    currentFilteredScores.forEach(scored => {
        const card = createScoredPropertyCard(scored, false);
        grid.appendChild(card);
    });
}

// Create Scored Property Card
function createScoredPropertyCard(scored, isDeansList) {
    const card = document.createElement('div');
    card.className = `scored-property-card ${isDeansList ? 'deans-list-property' : ''}`;
    card.onclick = () => showScoreDetail(scored);
    
    const property = scored.property;
    const address = property.address?.street || property.area_name || `Property ${scored.propertyIndex + 1}`;
    const price = formatCurrency(property.price);
    
    const gradeClass = getGradeClass(scored.grade);
    
    card.innerHTML = `
        <div class="score-card-header">
            <div class="score-card-grade grade-badge ${gradeClass}">${scored.grade}</div>
            <div class="score-card-gpa">
                <div class="gpa-label">GPA</div>
                <div class="gpa-value">${scored.gpa.toFixed(2)}</div>
            </div>
        </div>
        ...
        <div class="score-card-categories">
            ${createCategoryBar('Value', scored.categories.value, 'dollar-sign')}
            ${createCategoryBar('Location', scored.categories.location, 'map-marker-alt')}
            ${createCategoryBar('Property', scored.categories.property, 'home')}
            ${createCategoryBar('Market', scored.categories.market, 'chart-line')}
            ${createCategoryBar('Investment', scored.categories.investment, 'piggy-bank')}
            ${createCategoryBar('Appeal', scored.categories.appeal, 'star')}
        </div>
        ...
        <div class="score-card-footer">
            ...
            <div class="score-rank">#${scored.rank} of ${scoredProperties.length}</div>
        </div>
    `;
...
// Create Category Bar HTML
function createCategoryBar(name, value, icon) {
    return `
        <div class="category-score">
            ...
            <div class="category-bar-container">
                <div class="category-bar" style="width: ${value}%"></div>
            </div>
            <div class="category-value">${value.toFixed(0)}</div>
        </div>
    `;
}

// Show Score Detail Modal
function showScoreDetail(scored) {
    const property = scored.property;
    const address = property.address?.street || property.area_name || `Property ${scored.propertyIndex + 1}`;
    
    document.getElementById('scoreDetailTitle').textContent = address;
    document.getElementById('scoreDetailOverall').textContent = scored.overallScore.toFixed(1);
    
    const gradeElement = document.getElementById('scoreDetailGrade');
    gradeElement.textContent = scored.grade;
    gradeElement.className = `score-detail-grade grade-badge ${getGradeClass(scored.grade)}`;
    
    // Build breakdown
    const breakdownList = document.getElementById('scoreBreakdownList');
    breakdownList.innerHTML = `
        <div class="breakdown-item">
            <div class="breakdown-label"><i class="fas fa-dollar-sign"></i> Value Score</div>
            <div class="breakdown-value">${scored.categories.value.toFixed(1)}</div>
        </div>
        <div class="breakdown-item">
            <div class="breakdown-label"><i class="fas fa-map-marker-alt"></i> Location Score</div>
            <div class="breakdown-value">${scored.categories.location.toFixed(1)}</div>
        </div>
        <div class="breakdown-item">
            <div class="breakdown-label"><i class="fas fa-home"></i> Property Score</div>
            <div class="breakdown-value">${scored.categories.property.toFixed(1)}</div>
        </div>
        <div class="breakdown-item">
            <div class="breakdown-label"><i class="fas fa-chart-line"></i> Market Score</div>
            <div class="breakdown-value">${scored.categories.market.toFixed(1)}</div>
        </div>
        <div class="breakdown-item">
            <div class="breakdown-label"><i class="fas fa-piggy-bank"></i> Investment Score</div>
            <div class="breakdown-value">${scored.categories.investment.toFixed(1)}</div>
        </div>
        <div class="breakdown-item">
            <div class="breakdown-label"><i class="fas fa-star"></i> Appeal Score</div>
            <div class="breakdown-value">${scored.categories.appeal.toFixed(1)}</div>
        </div>
    `;
    
    // Generate insights
    generateScoreInsights(scored);
    
    // Build radar chart
    buildScoreRadarChart(scored);
...
function generateScoreInsights(scored) {
    const insights = [];
    ...
    insights.push(`<div class="insight-item"><i class="fas fa-arrow-up"></i> <strong>Strongest area:</strong> ${capitalizeFirst(strongest[0])} (${strongest[1].toFixed(0)}/100)</div>`);
    insights.push(`<div class="insight-item"><i class="fas fa-arrow-down"></i> <strong>Weakest area:</strong> ${capitalizeFirst(weakest[0])} (${weakest[1].toFixed(0)}/100)</div>`);
    
    // Grade-based insights
    if (scored.grade.startsWith('A')) {
        insights.push(`<div class="insight-item"><i class="fas fa-trophy"></i> This is an <strong>excellent property</strong> that scores well across all categories.</div>`);
    } else if (scored.grade.startsWith('B')) {
        insights.push(`<div class="insight-item"><i class="fas fa-thumbs-up"></i> This is a <strong>good property</strong> with solid fundamentals.</div>`);
    } else if (scored.grade.startsWith('C')) {
        insights.push(`<div class="insight-item"><i class="fas fa-balance-scale"></i> This is an <strong>average property</strong>. Consider if it meets your specific needs.</div>`);
    } else {
        insights.push(`<div class="insight-item"><i class="fas fa-exclamation-triangle"></i> This property has <strong>below-average scores</strong>. Review carefully before proceeding.</div>`);
    }
    
    // Rank-based insight
    if (scored.rank <= 3) {
        insights.push(`<div class="insight-item"><i class="fas fa-medal"></i> <strong>Top ${scored.rank} property</strong> in this search! Highly recommended.</div>`);
    }
    
    document.getElementById('scoreInsights').innerHTML = insights.join('');
}

// Build Score Radar Chart
function buildScoreRadarChart(scored) {
    ...
    scoreRadarChart = new Chart(ctx, {
        type: 'radar',
        data: {
            labels: ['Value', 'Location', 'Property', 'Market', 'Investment', 'Appeal'],
            datasets: [{
                label: 'Property Score',
                data: [
                    scored.categories.value,
                    scored.categories.location,
                    scored.categories.property,
                    scored.categories.market,
                    scored.categories.investment,
                    scored.categories.appeal
                ],
                backgroundColor: 'rgba(253, 119, 0, 0.2)',
                borderColor: '#fd7700',
                borderWidth: 3,
                pointBackgroundColor: '#fd7700',
                pointBorderColor: '#fff',
                pointHoverBackgroundColor: '#fff',
                pointHoverBorderColor: '#fd7700',
                pointRadius: 5,
                pointHoverRadius: 7
            }]
        },
...
function filterByGrade() {
    ...
    if (filter === 'all') {
        ...
        const filterIndex = gradeOrder.indexOf(filter);
        
        currentFilteredScores = scoredProperties.filter(p => {
            const gradeIndex = gradeOrder.indexOf(p.grade.charAt(0));
...
function sortByScore() {
    const sortBy = document.getElementById('scoreSort').value;
    
    switch(sortBy) {
        case 'score-desc':
            currentFilteredScores.sort((a, b) => b.overallScore - a.overallScore);
            break;
        case 'score-asc':
            currentFilteredScores.sort((a, b) => a.overallScore - b.overallScore);
            break;
        case 'value-desc':
            currentFilteredScores.sort((a, b) => b.categories.value - a.categories.value);
            break;
        case 'location-desc':
            currentFilteredScores.sort((a, b) => b.categories.location - a.categories.location);
            break;
        case 'property-desc':
            currentFilteredScores.sort((a, b) => b.categories.property - a.categories.property);
            break;
    }
